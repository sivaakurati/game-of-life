...
public class CellGridCanvas extends Canvas {
	...
	public void paint(Graphics g) {
		...
		Enumeration enumeration = cellGrid.getEnum();
		Cell c;
		while (enumeration.hasMoreElements()) {
			c = (Cell) enumeration.nextElement();
	...
	public synchronized void setShape(Shape shape) throws ShapeException {
		...
		if (dimShape.width > dimGrid.width || dimShape.height > dimGrid.height)
			throw new ShapeException("Shape doesn't fit on canvas (grid: " + dimGrid.width + "x" + dimGrid.height
					+ ", shape: " + dimShape.width + "x" + dimShape.height + ")");
